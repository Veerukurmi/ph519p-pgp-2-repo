program wave_function
    ...
    real(dp) :: E, x_beg, x_end
    integer(dp) :: grid, i,j,l
    !real :: start, finish
    
    
    real(dp), allocatable :: x(:) , u_wave(:), f(:), y(:), V(:), k(:), delta_h(:)

    grid = 100
    E = 0.5_dp ! Energy of the system in Atomic Units
    l = 6 ! l value for the effective potential
