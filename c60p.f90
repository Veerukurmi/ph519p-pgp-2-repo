...
real(dp) ::rmin, rmax, dr, energy, delx2, sigma
...
!allocation of arrays
 allocate(r(0:mesh),u(0:mesh),f(0:mesh),Uop(0:mesh))
...
do j =1, n
	...
	do i=0,mesh
			         ...
			         if (r(i)<r0) then
			               Uop(i)=0.0_dp-alphaa/(2*(r(i)**2+b**2)**2)

			              		    else if (r(i)>r0+del) then
			              		      Uop(i)=0.0_dp-alphaa/(2*(r(i)**2+b**2)**2)
				       else 
	              		 	Uop(i)=-0.302_dp-alphaa/(2*(r(i)**2+b**2)**2)

   
			              		 
				endif
				f(i)=1.0_dp-((delx2)*(alpha))*((l*(l+1))/(alpha*r(i)*r(i))+Uop(i)-energy)
